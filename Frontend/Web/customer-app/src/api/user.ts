...
export const userApi = {
  ...
  updateProfile: async (profileData: UpdateProfileRequest): Promise<ProfileUpdateResponse> => {
    try {
      ...
      if (error.statusCode === 500) {
        console.log(
          'Profile update endpoint returned 500 error - backend may not be fully configured'
        )
        throw new Error(
          'Profile update service is temporarily unavailable. Please try again later.'
        )
  ...
  changePassword: async (passwordData: ChangePasswordRequest): Promise<{ message: string }> => {
    try {
      ...
      if (error.statusCode === 500) {
        console.log(
          'Change password endpoint returned 500 error - backend may not be fully configured'
        )
        throw new Error(
          'Password change service is temporarily unavailable. Please try again later or contact support.'
        )
      } else if (error.statusCode === 404) {
  ...
  uploadAvatar: async (file: File): Promise<{ avatarUrl: string; message: string }> => {
    try {
      ...
      const payload: any = response.data
      const avatarUrl =
        payload.avatarUrl || payload.url || payload.data?.avatarUrl || payload.data?.url
      return {
        avatarUrl: avatarUrl,
        message:
          payload.message || payload.data?.message || 'Profile picture updated successfully!',
      }
    } catch (error: any) {
      ...
      if (error.statusCode === 500 || error.statusCode === 503) {
        ...
        console.log(
          'This likely means the FileStorageService or related dependencies are not properly configured'
        )
        ...
        return {
          avatarUrl,
          message:
            'Profile picture updated locally. Server upload will be available once backend is configured.',
        }
      } else if (error.statusCode === 404) {
  ...
  deleteAvatar: async (): Promise<{ message: string }> => {
    try {
      ...
      return {
        message:
          payload.message ?? payload.data?.message ?? 'Profile picture removed successfully!',
      }
    } catch (error: any) {
