...
# Telepesa Simple End-to-End Test
# Tests the complete infrastructure with proper user activation
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  Telepesa Simple E2E Test             ${NC}"
echo -e "${BLUE}========================================${NC}"
...
# Test User Service with activation
test_user_service() {
    ...
    docker exec -it telepesa-postgres psql -U telepesa -d telepesa_users_dev -c "UPDATE users SET status = 'ACTIVE' WHERE id = $USER_ID;" > /dev/null 2>&1
...
# Main test execution
main() {
    echo -e "Starting simple end-to-end tests..."
    echo -e ""
    ...
    echo -e "${GREEN}The Telepesa system is working correctly.${NC}"
}
