...
# Telepesa Direct Service Testing Script
# Tests the services directly since API Gateway routing is not working
...
test_user_registration() {
    ...
    local response=$(curl -s -w '%{http_code}' -X POST "$USER_SERVICE_URL/api/users/register" \
...
test_user_login() {
    ...
    local response=$(curl -s -w '%{http_code}' -X POST "$USER_SERVICE_URL/api/users/login" \
        -H 'Content-Type: application/json' \
        -d "$test_data")
...
test_create_savings_account() {
    ...
    local response=$(curl -s -w '%{http_code}' -X POST "$ACCOUNT_SERVICE_URL/api/accounts" \
        -H 'Content-Type: application/json' \
...
test_get_user_accounts() {
    local response=$(curl -s -w '%{http_code}' -X GET "$ACCOUNT_SERVICE_URL/api/accounts" \
        -H "Authorization: Bearer $JWT_TOKEN")
...
test_deposit_money() {
    ...
    local response=$(curl -s -w '%{http_code}' -X POST "$TRANSACTION_SERVICE_URL/api/transactions" \
        -H 'Content-Type: application/json' \
        -H "Authorization: Bearer $JWT_TOKEN" \
        -d "$test_data")
...
test_get_transaction_history() {
    local response=$(curl -s -w '%{http_code}' -X GET "$TRANSACTION_SERVICE_URL/api/transactions" \
        -H "Authorization: Bearer $JWT_TOKEN")
...
test_apply_for_loan() {
    ...
    local response=$(curl -s -w '%{http_code}' -X POST "$LOAN_SERVICE_URL/api/loans" \
        -H 'Content-Type: application/json' \
        -H "Authorization: Bearer $JWT_TOKEN" \
        -d "$test_data")
...
test_get_user_loans() {
    local response=$(curl -s -w '%{http_code}' -X GET "$LOAN_SERVICE_URL/api/loans/user/$USER_ID" \
        -H "Authorization: Bearer $JWT_TOKEN")
...
test_get_user_notifications() {
    local response=$(curl -s -w '%{http_code}' -X GET "$NOTIFICATION_SERVICE_URL/api/notifications/user/$USER_ID" \
        -H "Authorization: Bearer $JWT_TOKEN")
...
# Main test execution
main() {
    log_info "Starting Telepesa Direct Service Testing"
    log_info "Test User: $TEST_USERNAME"
    log_info "Testing services directly (bypassing API Gateway)"
    ...
    if [ $FAILED_TESTS -eq 0 ]; then
        ...
        log_warning "Note: API Gateway routing needs to be fixed for production use."
        exit 0
    else
