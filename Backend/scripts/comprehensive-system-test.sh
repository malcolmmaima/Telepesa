...
# Telepesa Comprehensive System Testing Script
...
make_request() {
    ...
    local curl_cmd="curl -s -w '%{http_code}' -X $method $GATEWAY_URL$endpoint"
...
test_user_login() {
    ...
    if [ "$status_code" = "200" ]; then
        JWT_TOKEN=$(extract_json_value "$response_body" "token")
        USER_ID=$(extract_json_number "$response_body" "userId")
...
test_create_savings_account() {
    ...
    if [ "$status_code" = "201" ]; then
        SAVINGS_ACCOUNT_ID=$(extract_json_number "$response_body" "id")
        SAVINGS_ACCOUNT_NUMBER=$(extract_json_value "$response_body" "accountNumber")
...
test_create_checking_account() {
    ...
    if [ "$status_code" = "201" ]; then
        CHECKING_ACCOUNT_ID=$(extract_json_number "$response_body" "id")
        CHECKING_ACCOUNT_NUMBER=$(extract_json_value "$response_body" "accountNumber")
...
test_apply_for_loan() {
    local test_data="{
        \"userId\": $USER_ID,
        \"accountNumber\": \"$SAVINGS_ACCOUNT_NUMBER\",
        \"loanType\": \"PERSONAL\",
...
# Main test execution
main() {
    log_info "Starting Telepesa Comprehensive System Testing"
    log_info "Test User: $TEST_USERNAME"
    log_info "Gateway URL: $GATEWAY_URL"
    
    # Check if services are running
    run_test "Service Health Check" "check_services" "0"
    
    # User Management Tests
    run_test "User Registration" "test_user_registration" "201"
    run_test "User Activation" "test_user_activation" "0"
    run_test "User Login" "test_user_login" "200"
    
    # Account Management Tests
    run_test "Create Savings Account" "test_create_savings_account" "201"
    run_test "Create Checking Account" "test_create_checking_account" "201"
    run_test "Get User Accounts" "test_get_user_accounts" "200"
    
    # Transaction Management Tests
    run_test "Deposit Money" "test_deposit_money" "201"
    run_test "Transfer Between Accounts" "test_transfer_between_accounts" "201"
    run_test "Get Transaction History" "test_get_transaction_history" "200"
    
    # Loan Management Tests
    run_test "Apply for Loan" "test_apply_for_loan" "201"
    run_test "Get User Loans" "test_get_user_loans" "200"
    
    # Notification Tests
    run_test "Get User Notifications" "test_get_user_notifications" "200"
    
    # Security Tests
    run_test "Unauthorized Access Test" "test_unauthorized_access" "401"
    run_test "Invalid JWT Test" "test_invalid_jwt" "401"
